...
hierarchical <- read_tsv("../instance/BREXIT_V20190522_DEDUP/query-results/pattern3-slot1-pattern24.tsv.gz")
```
...
```{r}
d <- full_join(both, hierarchical[, c("tweet_id", "match", "matchend", "query")], by = c("tweet_id", "match", "matchend"),
               suffix = c("", "_hierarchical"))
```
...
# library(DataEditR)
d %>% select(
  tweet_id, match, matchend, query, query_slot, query_hierarchical, 
  word,`0_word`, `1_word`, `0_word_slot`, `1_word_slot`,
  exact0, within0, overlap0, outside0,
  exact0_0, exact0_1, within0_0, within0_1, overlap0_0, overlap0_1, outside0_0, outside0_1,
  exact1, within1, overlap1, outside1,
  exact1_0, exact1_1, within1_0, within1_1, overlap1_0, overlap1_1, outside1_0, outside1_1
) %>% filter(is.na(query_hierarchical)) %>% filter(within1) %>% View
  # write_tsv("pattern3-slot1-pattern24-annotate.tsv", quote = "all")
